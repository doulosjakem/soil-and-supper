#!/usr/bin/env python3
"""
Processing template for Multi-Crop Disease Dataset.
...
This template activates when training_data/raw/multi_crop_disease/ exists
...
Critical class priorities:
  - Downy_mildew (currently 100% grapevine, needs diversity)
  - Anthracnose (needs more sources)
  - Rust (needs more sources)
"""
...
EXAMPLE_TAXONOMY_MAPPING = {
    ...
    "Corn_rust": {"class": "Rust", "confidence": "HIGH"},
    "Corn_healthy": {"class": "Healthy", "confidence": "HIGH"},
    "Downy_mildew": {"class": "Downy_mildew", "confidence": "HIGH"},
    "Anthracnose": {"class": "Anthracnose", "confidence": "HIGH"},
}
...
def discover_classes(dataset_dir: Path) -> Dict[str, List[Path]]:
    """Discover class directories. Supports nested crop/class layouts."""
...
def generate_intake_report(dataset_dir: Path) -> Dict:
    """Generate intake report for Multi-Crop Disease Dataset."""
    report = {
...
def print_report(report: Dict):
    print("=" * 80)
    print("MULTI-CROP DISEASE DATASET — INTAKE REPORT")
    print("=" * 80)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Multi-Crop Disease intake template")
